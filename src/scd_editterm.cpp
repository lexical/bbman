...
void SCD_EditTerm::Paste(const wxString& text, bool withANSI)
{
	...
	int bx, by;	//貼上前的游標位置
	getXY(&bx , &by);
	abs_AddLines( by+1 , newline_count );	//新增加行

	if( term_data[by][bx].getCharType() == TerminalChar::CH_WORDLAST )
	{	goLeft();	bx --;	}	//如果游標在中文字上，則左移一格
	...
	if( buf_len > 0 )
	{
		...
		for(int i=0;i<buf_len;i++)	buf[i] = term_data[by][bx+i];	//備份貼上前游標後面的字
	}

	DeleteLineTail();	//刪除貼上前游標後面的字

	EnableDrawing(false);
	parse(wrapped_txt);	//貼上字串
	EnableDrawing(true);
