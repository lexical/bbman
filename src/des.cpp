...
// this code is modified from libmcrypt,
...
/* $Id: des.cpp,v 1.1 2005/02/06 15:28:56 jakky1 Exp $ */
...
//	void *key;
	...
	keyword = (char*)calloc(1, _mcrypt_get_key_size());
	if (keyword == NULL)
		return -1;
	...
	key = (DES_KEY*)malloc(_mcrypt_get_size());
//	if (key == NULL)
	if (!key)
	{
		free(keyword);
	...
	_mcrypt_set_key(key, (char *) keyword, _mcrypt_get_key_size());
	free(keyword);

	_mcrypt_encrypt(key, (char *) ciphertext);
	...
	_mcrypt_decrypt(key, (char *) ciphertext);
	free(key);
...
int main()
{
	char *password = "gqugfho awifhiogu iosegu pseagk ukawhlefhaqilfjawlefjse lr";
	char buf[100];

	printf("enter a string :\n");
	scanf("%s", buf);

	SCD_des_encrypt( password , buf );
	printf("%s\n", buf);

	SCD_des_decrypt( password , buf );
	printf("%s\n", buf);
