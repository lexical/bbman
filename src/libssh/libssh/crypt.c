...
unsigned long packet_decrypt_len(SSH_SESSION *session, char *crypted){
    ...
    decrypted=(unsigned long *)crypted;
...
char * packet_encrypt(SSH_SESSION *session,void *data,unsigned int len){
    struct crypto_struct *crypto;
    HMAC_CTX *ctx;
    char *out;
    int finallen;
    unsigned long seq=ntohl(session->send_seq);
    if(!session->current_crypto)
    ...
    hmac_update(ctx,(unsigned char *)&seq,4);
...
int packet_hmac_verify(SSH_SESSION *session,BUFFER *buffer,char *mac){
    ...
    int len;
    unsigned long seq=htonl(session->recv_seq);
    ...
    ssh_print_hexa("seq",(unsigned char *)&seq,4);
#endif
