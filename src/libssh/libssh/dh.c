...
void ssh_get_random(void *where, int len){
	...
	char *buf = (char*)where;
	srand(time(0));
	for(i=0;i<len;i++) buf[i] = rand() % 256;
#else
	static int rndfd=0;
        if(!rndfd){
...
void make_sessionid(SSH_SESSION *session){
    ...
    buffer_add_long(session->in_hashbuf,0);
    buffer_add_char(session->in_hashbuf,0);
    buffer_add_long(session->out_hashbuf,0);
    buffer_add_char(session->out_hashbuf,0);
...
void hashbufout_add_cookie(SSH_SESSION *session){
    ...
    buffer_add_char(session->out_hashbuf,20);
...
void hashbufin_add_cookie(SSH_SESSION *session,unsigned char *cookie){
    ...
    buffer_add_char(session->in_hashbuf,20);
    ...
    switch(pubkey->type){
        case TYPE_DSS:
            ...
            if(valid==1)
                return 0;
            if(valid==-1){
                ssh_set_error(NULL,INVALID_DATA,"DSA error : %s",ERR_error_string(ERR_get_error(),NULL));
                return -1;
            }
            ssh_set_error(NULL,LIBSSH_NO_ERROR,"Invalid DSA signature");
            return -1;
        case TYPE_RSA:
        case TYPE_RSA1:
        valid=RSA_verify(NID_sha1,hash,SHA_DIGEST_LENGTH,
            ...
            if(valid==1)
                return 0;
            if(valid==-1){
                ssh_set_error(NULL,INVALID_DATA,"RSA error : %s",ERR_error_string(ERR_get_error(),NULL));
                return -1;
            }
            ssh_set_error(NULL,LIBSSH_NO_ERROR,"Invalid RSA signature");
            return -1;
       default:
            ssh_set_error(NULL,INVALID_DATA,"Unknown public key type");
            return -1;
   }
return -1;
}
...
int signature_verify(SSH_SESSION *session,STRING *signature){
    ...
    if(session->options->wanted_methods[KEX_HOSTKEY]){
         if(match(session->options->wanted_methods[KEX_HOSTKEY],pubkey->type_c)){
             ssh_set_error((session->connected?session:NULL),FATAL,"Public key from server (%s) doesn't match user preference (%s)",
    ...
    if(!sign){
        ssh_set_error((session->connected?session:NULL),INVALID_DATA,"Invalid signature blob");
        publickey_free(pubkey);
        return -1;
    }
