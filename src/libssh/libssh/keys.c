...
PUBLIC_KEY *publickey_make_dss(BUFFER *buffer){
    ...
    if(!p || !q || !g || !pubkey){
        ssh_set_error(NULL,FATAL,"Invalid DSA public key");
        if(p)
            free(p);
        if(q)
            free(q);
...
PUBLIC_KEY *publickey_make_rsa(BUFFER *buffer){
    ...
    buffer_free(buffer); /* we don't need it anymore */
    if(!e || !n){
        ssh_set_error(NULL,FATAL,"Invalid RSA public key");
        if(e)
            free(e);
        if(n)
            free(n);
...
PUBLIC_KEY *publickey_from_string(STRING *pubkey_s){
    ...
    if(!type_s){
        buffer_free(tmpbuf);
        ssh_set_error(NULL,FATAL,"Invalid public key format");
        return NULL;
    }
    ...
    ssh_set_error(NULL,FATAL,"unknown public key protocol %s",type);
    buffer_free(tmpbuf);
    free(type);
    return NULL;
}
...
SIGNATURE *signature_from_string(STRING *signature,PUBLIC_KEY *pubkey,int needed_type){
    ...
    type_s=buffer_get_ssh_string(tmpbuf);
    if(!type_s){
        ssh_set_error(NULL,FATAL,"Invalid signature packet");
        buffer_free(tmpbuf);
        return NULL;
    }
    type=string_to_char(type_s);
    ...
    switch(needed_type){
        case TYPE_DSS:
            if(strcmp(type,"ssh-dss")){
                ssh_set_error(NULL,FATAL,"Invalid signature type : %s",type);
                buffer_free(tmpbuf);
                free(type);
                return NULL;
            }
    ...
    switch(needed_type){
        ...
        case TYPE_RSA:
            ...
            if(len>rsalen){
                free(e);
                free(sign);
                ssh_set_error(NULL,FATAL,"signature too big ! %d instead of %d",len,rsalen);
                return NULL;
            }
            if(len<rsalen)
                ssh_say(0,"Len %d < %d\n",len,rsalen);
...
STRING *ssh_do_sign(SSH_SESSION *session,BUFFER *sigbuf, PRIVATE_KEY *privatekey){
    ...
    if(!sign->dsa_sign && !sign->rsa_sign){
        ssh_set_error(session,FATAL,"Signing : openssl error");
        return NULL;
    }
