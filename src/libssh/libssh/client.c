...
static char *get_banner(SSH_SESSION *session){
    char buffer[128];
    int i = 0;
    while (i < 127) {
#ifdef __WXMSW__
        if(recv(session->fd, &buffer[i], 1, 0)<=0){
#else
        if(read(session->fd, &buffer[i], 1)<=0){
#endif
            ssh_set_error(session,CONNECTION_LOST,"Remote host closed connection");
            return NULL;
        }
        if (buffer[i] == '\r')
            buffer[i] = 0;
    ...
    ssh_set_error(NULL,FATAL,"Too large banner");
    return NULL;
}

/* send_banner sends a SSH banner to the server */
/* TODO select a banner compatible with server version */
/* switch SSH1/1.5/2 */
/* and quits when the server is SSH1 only */

static void send_banner(SSH_SESSION *session){
...
int dh_handshake(SSH_SESSION *session){
    ...
    buffer_add_char(session->out_buffer,SSH2_MSG_KEXDH_INIT);
    ...
    packet_send(session);
    free(e);
    if(packet_wait(session,SSH2_MSG_KEXDH_REPLY))
        return -1;
    pubkey=buffer_get_ssh_string(session->in_buffer);
    if(!pubkey){
        ssh_set_error(NULL,FATAL,"No public key in packet");
        return -1;
    }
    dh_import_pubkey(session,pubkey);
    f=buffer_get_ssh_string(session->in_buffer);
    if(!f){
        ssh_set_error(NULL,FATAL,"No F number in packet");
        return -1;
    }
    dh_import_f(session,f);
    free(f);
    if(!(signature=buffer_get_ssh_string(session->in_buffer))){
        ssh_set_error(NULL,FATAL,"No signature in packet");
        return -1;
    }

    dh_build_k(session);
    packet_wait(session,SSH2_MSG_NEWKEYS);
    ssh_say(2,"Got SSH_MSG_NEWKEYS\n");
    packet_clear_out(session);
    buffer_add_char(session->out_buffer,SSH2_MSG_NEWKEYS);
    packet_send(session);
...
int ssh_service_request(SSH_SESSION *session,char *service){
    STRING *service_s;
    packet_clear_out(session);
    buffer_add_char(session->out_buffer,SSH2_MSG_SERVICE_REQUEST);
    ...
    if(packet_wait(session,SSH2_MSG_SERVICE_ACCEPT)){
        ssh_set_error(session,INVALID_DATA,"did not receive SERVICE_ACCEPT");
        return -1;
    }
...
SSH_SESSION *ssh_connect(SSH_OPTIONS *options){
  ...
  int fd;
  if(!options){
      ssh_set_error(NULL,FATAL,"Null argument given to ssh_connect !");
      return NULL;
  }
  ssh_crypto_init();
  if(options->fd==-1 && !options->host){
      ssh_set_error(NULL,FATAL,"Hostname required");
      return NULL;
  } 
  if(options->fd != -1)
      fd=options->fd;
  ...
  if(!(session->serverbanner=get_banner(session))){
      ssh_cleanup(session);
      return NULL;
  }
  ssh_say(2,"banner : %s\n",session->serverbanner);
  send_banner(session);
  if(ssh_get_kex(session)){
...
static void ssh_cleanup(SSH_SESSION *session){
    ...
    memset(session,0,sizeof(SSH_SESSION)); /* burn connection, it could hangs sensitive datas */
    free(session);
}
...
void ssh_disconnect(SSH_SESSION *session){
    ...
    if(session->fd!= -1) {
        packet_clear_out(session);
        buffer_add_char(session->out_buffer,SSH2_MSG_DISCONNECT);
        buffer_add_long(session->out_buffer,htonl(SSH2_DISCONNECT_BY_APPLICATION));
