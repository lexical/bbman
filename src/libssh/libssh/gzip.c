...
static z_stream *initcompress(SSH_SESSION *session,int level){
    ...
    if (status!=0)
        ssh_set_error((session->connected?session:NULL),FATAL,"status %d inititalising zlib deflate",status);
    return stream;
}
...
BUFFER *gzip_compress(SSH_SESSION *session,BUFFER *source,int level){
    ...
    do {
        ...
        if(status !=0){
            ssh_set_error((session->connected?session:NULL),FATAL,"status %d deflating zlib packet",status);
            return NULL;
        }
...
static z_stream *initdecompress(SSH_SESSION *session){
    ...
    if (status!=0){
        ssh_set_error((session->connected?session:NULL),FATAL,"Status = %d initiating inflate context !",status);
        free(stream);
        stream=NULL;
    }
    return stream;
...
BUFFER *gzip_decompress(SSH_SESSION *session,BUFFER *source){
    ...
    do {
        ...
        if(status !=Z_OK){
            ssh_set_error((session->connected?session:NULL),FATAL,"status %d inflating zlib packet",status);
            buffer_free(dest);
            return NULL;
        }
