...
int sftp_packet_write(SFTP_SESSION *sftp,unsigned char type, BUFFER *payload){
    unsigned long size;
    buffer_add_data_begin(payload,&type,1);
    size=htonl(buffer_get_len(payload));
    buffer_add_data_begin(payload,&size,4);
    return channel_write(sftp->channel,buffer_get(payload),buffer_get_len(payload));
}
...
SFTP_MESSAGE *sftp_get_message(SFTP_PACKET *packet){
    ...
    && (packet->type != SSH_FXP_NAME)){
        ssh_set_error(packet->sftp->session,INVALID_DATA,"get_message : unknown packet type %d\n",packet->type);
        sftp_message_free(msg);
        return NULL;
    }
    if(buffer_get_long(packet->payload,&msg->id)!=sizeof(long)){
        ssh_set_error(packet->sftp->session,INVALID_DATA,"invalid packet %d : no ID",packet->type);
        sftp_message_free(msg);
        return NULL;
    }
...
int sftp_init(SFTP_SESSION *sftp){
    ...
    unsigned long version=htonl(LIBSFTP_VERSION);
    buffer_add_long(buffer,version);
    ...
    if(packet->type != SSH_FXP_VERSION){
        ssh_set_error(sftp->session,INVALID_DATA,"Received a %d messages instead of SSH_FXP_VERSION",packet->type);
        sftp_packet_free(packet);
        return -1;
    }
...
SFTP_MESSAGE *sftp_dequeue(SFTP_SESSION *session, unsigned long id){
...
unsigned long sftp_get_new_id(SFTP_SESSION *session){
...
STATUS_MESSAGE *parse_status_msg(SFTP_MESSAGE *msg){
    ...
    if(msg->packet_type != SSH_FXP_STATUS){
        ssh_set_error(msg->sftp->session, INVALID_DATA,"Not a ssh_fxp_status message passed in !");
        return NULL;
    }
    ...
    if( (buffer_get_long(msg->payload,&status->status)!= 4)
     ...
     !(status->lang=buffer_get_ssh_string(msg->payload))){
         ...
         free(status);
         ssh_set_error(msg->sftp->session,INVALID_DATA,"invalid SSH_FXP_STATUS message");
         return NULL;
    }
...
SFTP_FILE *parse_handle_msg(SFTP_MESSAGE *msg){
    ...
    if(msg->packet_type != SSH_FXP_HANDLE){
        ssh_set_error(msg->sftp->session,INVALID_DATA,"Not a ssh_fxp_handle message passed in !");
        return NULL;
    }
    ...
    file->eof=0;
    if(!file->handle){
        ssh_set_error(msg->sftp->session,INVALID_DATA,"Invalid SSH_FXP_HANDLE message");
        free(file);
        return NULL;
    }
    return file;
...
SFTP_DIR *sftp_opendir(SFTP_SESSION *sftp, char *path){
    ...
    unsigned long id=sftp_get_new_id(sftp);
    buffer_add_long(payload,id);
    ...
    switch (msg->packet_type){
        case SSH_FXP_STATUS:
            ...
            if(!status)
                return NULL;
            ssh_set_error(sftp->session,REQUEST_DENIED,"sftp server : %s",status->errormsg);
        ...
        case SSH_FXP_HANDLE:
            ...
            return dir;
        default:
            ssh_set_error(sftp->session,INVALID_DATA,"Received message %d during opendir!",msg->packet_type);
            sftp_message_free(msg);
    }
    return NULL;
}
...
SFTP_ATTRIBUTES *sftp_parse_attr_4(SFTP_SESSION *sftp,BUFFER *buf){
    ...
    if(!ok){
        ...
        free(attr);
        ssh_set_error(sftp->session,INVALID_DATA,"Invalid ATTR structure");
        return NULL;
    }
...
SFTP_ATTRIBUTES *sftp_parse_attr_3(SFTP_SESSION *sftp,BUFFER *buf){
    unsigned long flags=0;
    STRING *name;
    STRING *longname;
    SFTP_ATTRIBUTES *attr=malloc(sizeof(SFTP_ATTRIBUTES));
...
void buffer_add_attributes(BUFFER *buffer, SFTP_ATTRIBUTES *attr){
	unsigned long flags=(attr?attr->flags:0);
	flags &= (SSH_FILEXFER_ATTR_SIZE | SSH_FILEXFER_ATTR_UIDGID | SSH_FILEXFER_ATTR_PERMISSIONS | SSH_FILEXFER_ATTR_ACMODTIME);
	buffer_add_long(buffer,htonl(flags));
	if(attr){
		if (flags & SSH_FILEXFER_ATTR_SIZE)
		{
			buffer_add_longlong(buffer, htonll(attr->size));
		}
		if(flags & SSH_FILEXFER_ATTR_UIDGID){
			buffer_add_long(buffer,htonl(attr->uid));
			buffer_add_long(buffer,htonl(attr->gid));
		}
		if(flags & SSH_FILEXFER_ATTR_PERMISSIONS){
			buffer_add_long(buffer,htonl(attr->permissions));
		}
		if (flags & SSH_FILEXFER_ATTR_ACMODTIME)
		{
			buffer_add_long(buffer, htonl(attr->atime));
			buffer_add_long(buffer, htonl(attr->mtime));
		}
	}
}
...
SFTP_ATTRIBUTES *sftp_parse_attr(SFTP_SESSION *session, BUFFER *buf){
    switch(session->server_version){
        case 4:
            return sftp_parse_attr_4(session,buf);
        case 3:
            return sftp_parse_attr_3(session,buf);
        default:
            ssh_set_error(session->session,INVALID_DATA,"Version %d unsupported by client",session->server_version);
            return NULL;
    }
    return NULL;
}
...
SFTP_ATTRIBUTES *sftp_readdir(SFTP_SESSION *sftp, SFTP_DIR *dir){
    ...
    if(!dir->buffer){
        ...
        switch (msg->packet_type){
            case SSH_FXP_STATUS:
                ...
                ssh_set_error(sftp->session,INVALID_DATA,"Unknown error status : %d",status->status);
                status_msg_free(status);
                return NULL;
            case SSH_FXP_NAME:
                ...
                break;
            default:
                ssh_set_error(sftp->session,INVALID_DATA,"unsupported message back %d",msg->packet_type);
                sftp_message_free(msg);
                return NULL;
        }
    }
    ...
    if(dir->count==0){
        ssh_set_error(sftp->session,INVALID_DATA,"Count of files sent by the server is zero, which is invalid, or libsftp bug");
        return NULL;
    }
    ssh_say(2,"Count is %d\n",dir->count);
    attr=sftp_parse_attr(sftp,dir->buffer);
    dir->count--;
    if(dir->count==0){
...
static int sftp_handle_close(SFTP_SESSION *sftp, STRING *handle){
    ...
    switch (msg->packet_type){
        case SSH_FXP_STATUS:
            ...
            return err;
        default:
            ssh_set_error(sftp->session,INVALID_DATA,"Received message %d during sftp_handle_close!",msg->packet_type);
            sftp_message_free(msg);
    }
    return -1;
}
...
SFTP_FILE *sftp_open(SFTP_SESSION *sftp, char *file, int access, SFTP_ATTRIBUTES *attr){
    ...
    switch (msg->packet_type){
        ...
        case SSH_FXP_HANDLE:
            ...
            return handle;
        default:
            ssh_set_error(sftp->session,INVALID_DATA,"Received message %d during open!",msg->packet_type);
            sftp_message_free(msg);
    }
    return NULL; 
}
...
int sftp_read(SFTP_FILE *handle, void *data, int len){
    ...
    buffer_add_longlong(buffer,htonll(handle->offset));
    buffer_add_long(buffer,htonl(len));
    ...
    switch (msg->packet_type){
        ...
        case SSH_FXP_DATA:
            ...
            sftp_message_free(msg);
            if(!datastring){
                ssh_set_error(sftp->session,INVALID_DATA,"Received invalid DATA packet from sftp server");
                return -1;
            }
            if(string_len(datastring)>len){
                ssh_set_error(sftp->session,INVALID_DATA,"Received a too big DATA packet from sftp server : %d and asked for %d",
            ...
            return len;
        default:
            ssh_set_error(sftp->session,INVALID_DATA,"Received message %d during read!",msg->packet_type);
            sftp_message_free(msg);
            return -1;
    }
...
int sftp_write(SFTP_FILE *file, void *data, int len){
    ...
    switch (msg->packet_type){
        case SSH_FXP_STATUS:
            ...
            return (err?err:len);
        default:
            ssh_set_error(sftp->session,INVALID_DATA,"Received message %d during write!",msg->packet_type);
            sftp_message_free(msg);
            return -1;
    }
    return -1; /* not reached */
...
int sftp_rm(SFTP_SESSION *sftp, char *file) {
    ...
    if (msg->packet_type == SSH_FXP_STATUS) {
        ...
        ssh_set_error(sftp->session,INVALID_DATA, "Received message %d when attempting to remove file", msg->packet_type);
        sftp_message_free(msg);
    }
    return -1;
}
...
int sftp_rmdir(SFTP_SESSION *sftp, char *directory) {
    ...
    {
        ssh_set_error(sftp->session,INVALID_DATA, "Received message %d when attempting to remove directory", msg->packet_type);
        sftp_message_free(msg);
    }
    return -1;
}
...
int sftp_mkdir(SFTP_SESSION *sftp, char *directory, SFTP_ATTRIBUTES *attr) {
    ...
    if (msg->packet_type == SSH_FXP_STATUS) {
       ...
       ssh_set_error(sftp->session,INVALID_DATA, "Received message %d when attempting to make directory", msg->packet_type);
       sftp_message_free(msg);
    }
    return -1;
}
...
int sftp_rename(SFTP_SESSION *sftp, char *original, char *newname) {
    ...
    if (msg->packet_type == SSH_FXP_STATUS) {
        ...
        ssh_set_error(sftp->session,INVALID_DATA, "Received message %d when attempting to rename", msg->packet_type);
        sftp_message_free(msg);
    }
    return -1;
}
...
int sftp_setstat(SFTP_SESSION *sftp, char *file, SFTP_ATTRIBUTES *attr) {
    ...
    if (msg->packet_type == SSH_FXP_STATUS) {
        ...
        ssh_set_error(sftp->session,INVALID_DATA, "Received message %d when attempting to set stats", msg->packet_type);
        sftp_message_free(msg);
    }
    return -1;
}
...
char *sftp_canonicalize_path(SFTP_SESSION *sftp, char *path)
{
	...
	{
		buffer_get_long(msg->payload, &ignored);	/* we don't care about "count" */
