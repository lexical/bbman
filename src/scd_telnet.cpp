...
// ----------------------------------------------------------------------------
void SCD_Telnet::MarkKeepAliveSent()
{
	UnIdleUser();
}
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void SCD_Telnet::Paste(const wxString& text, bool withANSI)
{
	wxString wrapped = GetLineWrapedString(text);
	wxCharBuffer big5_buf = wxStringToBig5Buffer(wrapped);
	char *w = const_cast<char*>(big5_buf.data());
	if( ! w ) return;

	if( withANSI )	//如果需要拷貝 ANSI code
	{
		while( *w != '\0' )
		{
			if( *w == '\x1b' )	*w = '\x15';
	...
	{
		char *r = const_cast<char*>(big5_buf.data());
		while( *r != '\0' )
		{
			if( *r == '\x1b' && r[1] == '[' )	//跳過 ANSI code
			{
				r++;
				while(1)
				{
