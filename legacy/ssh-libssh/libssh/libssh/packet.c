...
int packet_read(SSH_SESSION *session){
    ...
    if(be_read!=blocksize){
        if(be_read<=0){
            ...
            close(session->fd);
            session->fd=-1;
            ssh_set_error((session->connected?session:NULL),FATAL,
            (be_read==0)?"Connection closed by remote host" : "Error reading socket");
            return -1;
        }
        ssh_set_error((session->connected?session:NULL),FATAL,"read_packet(): asked %d bytes, received %d",blocksize,be_read);
        return -1;
    }
    ...
    if(len> MAX_PACKET_LEN){
        ssh_set_error((session->connected?session:NULL),FATAL,"read_packet(): Packet len too high(%uld)",len);
        return -1;
    }
    to_be_read=len-be_read+4;
    if(to_be_read<0){
        /* remote sshd is trying to get me ?*/
        ssh_set_error((session->connected?session:NULL),FATAL,"given numbers of bytes left to be read <0 (%d)!",to_be_read);
        return -1;
    }
    ...
    if(to_be_read !=0){
        ...
        if(i<=0){
            ...
            close(session->fd);
            session->fd=-1;
            ssh_set_error((session->connected?session:NULL),FATAL,"Server closed connection");
            return -1;
        }
        if(i!=to_be_read){
            free(packet);
            ...
            ssh_set_error((session->connected?session:NULL),FATAL,"read_packet(): read only %d, wanted %d",i,to_be_read);
            return -1;
        }
    ...
    if(session->current_crypto){
        ...
        if((i=completeread(session->fd,mac,macsize))!=macsize){
            ...
            ssh_set_error((session->connected?session:NULL),FATAL,"read_packet(): wanted %d, had %d",i,macsize);
            return -1;
        }
        if(packet_hmac_verify(session,session->in_buffer,mac)){
            ssh_set_error((session->connected?session:NULL),FATAL,"HMAC error");
            return -1;
        }
    }
    buffer_pass_bytes(session->in_buffer,4);   /*pass the size which has been processed before*/
    if(!buffer_get_char(session->in_buffer,&padding)){
        ssh_set_error((session->connected?session:NULL),FATAL,"Packet too short to read padding");
        return -1;
    }
    ssh_say(3,"%d bytes padding\n",padding);
    if(padding > buffer_get_rest_len(session->in_buffer)){
        ssh_set_error((session->connected?session:NULL),FATAL,"invalid padding: %d (%d resting)",padding,buffer_get_rest_len(session->in_buffer));
...
int packet_translate(SSH_SESSION *session){
    ...
    if(!buffer_get_char(session->in_buffer,&session->in_packet.type)){
        ssh_set_error((session->connected?session:NULL),FATAL,"Packet too short to read type");
        return -1;
    }
...
void packet_send(SSH_SESSION *session){
    char padstring[32];
    unsigned long finallen;
    unsigned char padding;
    unsigned long currentlen=buffer_get_len(session->out_buffer);
    char *hmac;
    ...
    buffer_add_data_begin(session->out_buffer,&padding,1);
    buffer_add_data_begin(session->out_buffer,&finallen,4);
...
#ifdef __WXMSW__
    if(send(session->fd,buffer_get(session->out_buffer),buffer_get_len(session->out_buffer), 0)!=buffer_get_len(session->out_buffer)){
#else
    if(write(session->fd,buffer_get(session->out_buffer),buffer_get_len(session->out_buffer))!=buffer_get_len(session->out_buffer)){
#endif
        session->alive=0;
        close(session->fd);
        session->fd=-1;
        ssh_set_error((session->connected?session:NULL),FATAL,"Writing packet : error on socket (or connection closed)");
    }
    session->send_seq++;
...
void packet_parse(SSH_SESSION *session){
    ...
    switch(type){
        case SSH2_MSG_DISCONNECT:
            ...
            ssh_set_error((session->connected?session:NULL),FATAL,"Received SSH_MSG_DISCONNECT : %s",error);
            if(error_s){
                free(error_s);
                free(error);
            }
        ...
        switch(session->in_packet.type){
           ...
           default:
               if(type && (type != session->in_packet.type)){
                   ssh_set_error((session->connected?session:NULL),FATAL,"waitpacket(): Received a %d type packet, was waiting for a %d\n",session->in_packet.type,type);
                   return -1;
               }
               return 0;
           }
